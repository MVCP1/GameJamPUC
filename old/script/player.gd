...
func _input(event):
	...
	if event is InputEventMouseMotion:
		...
		get_node("Camera").set_rotation(Vector3(deg_to_rad(clamp(rad_to_deg(get_node("Camera").get_rotation().x), -ANGLE, ANGLE)), get_node("Camera").get_rotation().y, get_node("Camera").get_rotation().z))
...
func _physics_process(delta):
	...
	if not on_floor:
		velocity = Vector3(inertia.x,move.y,inertia.z)
		move_and_slide()
	else:
		velocity = move
		move_and_slide()
...
#DEBUG INFORMATION
	#MOVEMENT VECTOR
	$Camera/CanvasLayer/RichTextLabel3.text = str(move)
	#INERTIA VECTOR
	$Camera/CanvasLayer/RichTextLabel7.text = str(inertia)
	#ON FLOOR (3x)
	if is_on_floor():
